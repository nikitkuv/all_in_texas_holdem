...
# 1) ДОБАВИТЬ ЗАДЕРЖКУ ДЛЯ ПОКАЗА ВСЕХ КАРТ (ПО ОЧЕРЕДИ НЕ МГНОВЕННО ПОКАЗЫВАЮТСЯ ВСЕ КАРТЫ) И ТО ЖЕ САМОЕ ДЛЯ ПОКАЗА ПРОМЕЖУТОЧНЫХ РЕЗУЛЬТАТОВ
# 2) НАЙТИ БИБЛИОТЕКУ ИЛИ ВСТРОЕННУЮ ФУНКЦИЮ, УБИРАЮЩУЮ ТЕКСТ С ЭКРАНА (ДЛЯ ТОГО, ЧТОБЫ УБИРАТЬ КАРТЫ ПРЕДЫДУЩЕГО ИГРОКА ПРИ ВЫБОРЕ CALL OR FOLD)
# 3) СДЕЛАТЬ ЗАСЕЧЕНИЕ ВРЕМЕНИ ИГРЫ И ЗАПИСЫВАТЬ В ФАЙЛ РЕКОРДЫ ПО ВРЕМЕНИ ИГРЫ И РЕДКИЕ КОМБИНАЦИИ, СДЕЛАТЬ ТАБЛИЦУ, ИСПОЛЬЗУЯ PANDAS
# 4) ЗАМЕНИТЬ КОРОТКИЕ ЦИКЛЫ FOR НА ФУНКЦИЮ MAP() В НАПИСАННЫХ ФУНКЦИЯХ
...
while len(players) != 1:
    ...
    if len(in_game) > 1:
        ...
        for index, i in enumerate(round_results): #[306, 'Two Pairs', [5, 13]]
            i.insert(0, in_game[index][0])
            i.insert(5, in_game[index][1]) #['Name', 306, 'Two Pairs', [5, 13], 1000]
        sorted_rr = pf.winner_list(round_results) #sorting players in terms of the power of combinations they gathered
